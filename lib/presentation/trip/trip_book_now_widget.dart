...
class _TripBookNowWidgetState extends State<TripBookNowWidget> {
    ...
    setState(() {
      if (key == 0) {
        ...
        if (_selectedDate1 != null &&
            (_selectedDate1!.isBefore(_selectedDate0!) ||
                _selectedDate1!.isAtSameMomentAs(_selectedDate0!))) {
          _selectedDate1 = _selectedDate0!.add(const Duration(days: 1));
        }
      } else if (key == 1) {
        if (_selectedDate0 != null &&
            (picked.isBefore(_selectedDate0!) ||
                picked.isAtSameMomentAs(_selectedDate0!))) {
  ...
  void _onPressedBookNow(PropertyModel property, UserModel user) {
    ...
    final taxes =
        ((property.price ?? 0) * datesCal) * (property.taxRate ?? 0.0) / 100;
    final totalPrice =
        ...
        BlocListener<TripBloc, TripState>(
          listener: (context, state) {
            state.maybeWhen(
              ...
              addTripSuccess: (data) async {
                context.read<MessageCubit>().showSuccessMessage(
                  '예약이 완료 되었습니다.',
                );
                String tripIdText = Uri.encodeComponent(data.tripId.toString());
